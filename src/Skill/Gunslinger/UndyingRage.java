...
public class UndyingRage extends Skill {

	private static final double MANA_COST = 5, COOLDOWN = 6000, HEAL_PERCENT = 20;

	private Player p;

	public UndyingRage(Player p, char c) {
		super(c);
		this.p = p;
		this.setManaCost(UndyingRage.MANA_COST).setCooldown(UndyingRage.COOLDOWN);
	}
	...
	public String getInfo() {
		return String.format("Undying Rage: Restores %.1f percent of the player's health", HEAL_PERCENT);
	}
	...
	public BufferedImage getImage(int x, int y) {
		...
		g.drawImage(ImageLoader.getSkillImage(ImageLoader.UNDYING_RAGE), 0, 0, x, y, null);
