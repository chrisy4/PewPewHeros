...
public class TankUp extends Skill {

	private static final double DEFENSE_BOOST_PERCENT = 25, MANA_COST = 5, COOLDOWN = 5000, BOOST_DURATION = 3000;
	...
	public TankUp(Player p, char c) {
		super(c);
		this.p = p;
		this.setManaCost(TankUp.MANA_COST).setCooldown(TankUp.COOLDOWN);
	}
	...
	public String getInfo() {
		return String.format("Tank Up: Grants player a defense boost of %.1f percent for %.1f seconds.",
				DEFENSE_BOOST_PERCENT, BOOST_DURATION / 1000);
	}
	...
	public Skill onUse(Level l) {
		...
		l.getPlayer().increaseDefense(DEFENSE_BOOST_PERCENT);
		removed = false;
		return this;
	}
	...
	public BufferedImage getImage(int x, int y) {
		...
		g.drawImage(ImageLoader.getSkillImage(ImageLoader.TANK_UP), 0, 0, x, y, null);
	...
	public Skill update(Level l) {
		...
		if (boostCount == 0 && !removed) {
			removed = true;
			l.getPlayer().decreaseDefense(DEFENSE_BOOST_PERCENT);
		}
		return this;
	}
}